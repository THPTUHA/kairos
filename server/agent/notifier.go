...
func (n *notifier) sendExecutionEmail() error {
	...
	e := &email.Email{
		...
		Subject: fmt.Sprintf("%s%s %s execution report", n.AgentConfig.MailSubjectPrefix, n.statusString(), n.Execution.JobName),
...
func SendPostNotifications(config *AgentConfig, execution *Execution, exGroup []*Execution, job *Job, logger *logrus.Entry) error {
	n := &notifier{
		logger: logger,
	...
	if n.AgentConfig.MailHost != "" && n.AgentConfig.MailPort != 0 && n.Job.OwnerEmail != "" {
