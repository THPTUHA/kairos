...
type Plugins struct {
	Processors map[string]kplugin.Processor
...
func (p *Plugins) DiscoverPlugins() error {
	p.Processors = make(map[string]kplugin.Processor)
	...
	log.Info().Msg(fmt.Sprintf("executors %+v:", executors))
	for _, file := range processors {
		...
		p.Processors[pluginName] = raw.(kplugin.Processor)
	}
