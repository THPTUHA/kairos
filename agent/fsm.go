...
const (
	// SetTaskType is the command used to store a task in the store.
	SetTaskType MessageType = iota
	// DeleteTaskType is the command used to delete a Task from the store.
	DeleteTaskType
...
func (d *kairosFSM) Apply(l *raft.Log) interface{} {
	...
	switch msgType {
	case SetTaskType:
		return d.applySetTask(buf[1:])
	case DeleteTaskType:
		return d.applyDeleteTask(buf[1:])
...
func (d *kairosFSM) applySetTask(buf []byte) interface{} {
	var pj kproto.Task
	if err := proto.Unmarshal(buf, &pj); err != nil {
		return err
	}
	task := NewTaskFromProto(&pj, d.logger)
	if err := d.store.SetTask(task); err != nil {
		return err
	}
	return nil
}

func (d *kairosFSM) applyDeleteTask(buf []byte) interface{} {
	var djr kproto.DeleteTaskRequest
	if err := proto.Unmarshal(buf, &djr); err != nil {
		return err
	}
	task, err := d.store.DeleteTask(djr.GetTaskId())
	if err != nil {
		return err
	}
	return task
