...
func (a *Agent) setupRaft() error {
	...
	if a.config.DevMode {
		...
		if _, err := os.Stat(peersFile); err == nil {
			...
			store, err := NewStore(a.logger, false)
			if err != nil {
...
// AgentConfig returns the agent's config.
...
// applySetTask is a helper method to be called when
// a task property need to be modified from the leader.
func (a *Agent) applySetTask(task *proto.Task) error {
	cmd, err := Encode(SetTaskType, task)
	if err != nil {
		return err
	}
...
// GetRunningTasks returns amount of active tasks of the local agent
...
// func (a *Agent) recursiveSetTask(tasks []*Task) []string {
// 	result := make([]string, 0)
// 	for _, task := range tasks {
// 		err := a.GRPCClient.SetTask(task)
// 		if err != nil {
// 			result = append(result, "fail create "+task.Name)
// 			continue
// 		} else {
// 			result = append(result, "success create "+task.Name)
// 			if len(task.ChildTasks) > 0 {
// 				recursiveResult := a.recursiveSetTask(task.ChildTasks)
