...
type statusAgentHelper struct {
	...
	stream    proto.Agent_AgentRunServer
}
...
// AgentRun is called when an agent starts running a task and lasts all execution,
// the agent will stream execution progress to the server.
func (as *AgentServer) AgentRun(req *proto.AgentRunRequest, stream proto.Agent_AgentRunServer) error {
	...
	if executor, ok := as.agent.ExecutorPlugins[jex]; ok {
		...
		if err != nil {
			as.logger.WithError(err).WithField("task", task.Key).WithField("plugin", executor).Error("grpc_agent: command error output")
			success = false
	...
	if err := stream.Send(&proto.AgentRunStream{
		...
		as.logger.WithError(err).WithField("task", task.Key).Error("grpc_agent: error sending the final execution, falling back to ExecutionDone")
