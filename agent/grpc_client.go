...
// AgentRun runs a task in the given agent
func (grpcc *GRPCClient) AgentRun(addr string, task *proto.Task, execution *proto.Execution) error {
	...
	for {
		...
		grpcc.logger.WithField("key", ars.Execution.Key()).Info("grpc: received execution stream")
...
func (grpcc *GRPCClient) SetTask(task *Task) error {
	var conn *grpc.ClientConn
	...
	// Synchronous call
	d := proto.NewKairosClient(conn)
	_, err = d.SetTask(context.Background(), &proto.SetTaskRequest{
		Task: task.ToProto(),
	})
	if err != nil {
		grpcc.logger.WithError(err).WithFields(logrus.Fields{
...
// GetTask calls GetTask gRPC method in the server
func (grpcc *GRPCClient) GetTask(addr string, taskID int64) (*Task, error) {
	var conn *grpc.ClientConn
	...
	// Synchronous call
	d := proto.NewKairosClient(conn)
	gjr, err := d.GetTask(context.Background(), &proto.GetTaskRequest{TaskId: taskID})
	if err != nil {
		grpcc.logger.WithError(err).WithFields(logrus.Fields{
			"method":      "GetTask",
			"server_addr": addr,
	...
	return NewTaskFromProto(gjr.Task, grpcc.logger), nil
}
